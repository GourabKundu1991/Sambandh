...
const MemberBaseDetailsScreen = ({ navigation, route }) => {
    ...
    const getAllData = () => {
        AsyncStorage.getItem('userToken').then(val => {
            if (val != null) {
                ...
                formdata.append("memberType", route.params.baseName);
                formdata.append("type", route.params.baseType);
                formdata.append("searchText", searchTerm);
                formdata.append("page", 1);
                console.log(formdata);
                fetch(`${BASE_URL}/get_dashboard_member_type_details`, {
                    method: 'POST',
                    headers: {
                    ...
                    .then((responseJson) => {
                        console.log("get_dashboard_member_type_details:", responseJson);
                        if (responseJson.bstatus == 1) {
                            setLoading(false);
                            setMemberList(responseJson.details);
                        } else {
    ...
    return (
        <NativeBaseProvider>
            ...
            <VStack flex={1} backgroundColor={lightColor}>
                <HeaderComponents component={route.params.baseName} themeColor={orgDetails.color} navigation={navigation} />
                <ScrollView automaticallyAdjustKeyboardInsets={true}>
                    <VStack padding={5} space={6}>
                        <View style={MainStyle.inputbox}>
                            <Input fontFamily={fontBold} size="md" variant="unstyled" placeholder={t('Code / Name / Phone')}
                                ...
                                <VStack space={3}>
                                    ...
                                    <HStack justifyContent={'space-between'} alignItems={'center'}>
                                        <VStack paddingX={3} width={'70%'}>
                                            ...
                                            <Pressable style={{ marginTop: 10 }} onPress={() => Linking.openURL(`tel:${item.phone_number}`)}>
                                                <HStack alignItems={'center'} space={2}>
                                                    ...
                                                    <Text fontFamily={fontBold} color={orgDetails.color}>{item.phone_number}</Text>
                                                </HStack>
                                            </Pressable>
                                        </VStack>
                                        <Button variant={'unstyled'} size={'sm'} style={{ width: 90 }} borderRadius={30} overflow={'hidden'}>
                                            <Text color={successColor} fontWeight={'bold'} fontSize="lg">{t('Open')}</Text>
                                        </Button>
                                    </HStack>
                                    <HStack justifyContent="space-between" backgroundColor={lightGrey} paddingY={3} paddingX={5} borderRadius={30} overflow={'hidden'}>
                                        <Text fontSize="sm" color={darkGrey} fontFamily={fontBold}>
                                            {t("Available Points")}:
                                        </Text>
                                        <Text fontSize="sm" color={darkColor} fontWeight="bold">
                                            {item.available_point} Points
                                        </Text>
                                    </HStack>
                                </VStack>
                            </LinearGradient>
...
export default MemberBaseDetailsScreen;